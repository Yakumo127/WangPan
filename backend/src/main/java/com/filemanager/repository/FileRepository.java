...
public interface FileRepository extends JpaRepository<File, Long> {
    ...
    @Query("SELECT f FROM File f JOIN f.user u WHERE f.deleted = true AND f.ownerHidden = true " +
           ...
           "AND ( :kw IS NULL OR LOWER(f.originalFilename) LIKE LOWER(CONCAT('%',:kw,'%')) OR LOWER(u.username) LIKE LOWER(CONCAT('%',:kw,'%')) ) " +
           "AND ( :reason IS NULL OR LOWER(f.adminDeleteReason) LIKE LOWER(CONCAT('%',:reason,'%')) ) " +
    ...
    // 通过 SHA-256 哈希查找（用于秒传：限定当前用户，以避免跨用户物理路径复用问题）
    @Query("SELECT f FROM File f WHERE f.fileHash = :hash AND f.deleted = false AND f.user.id = :userId")
    Optional<File> findFirstByFileHashAndUserIdAndDeletedFalse(@Param("hash") String fileHash, @Param("userId") Long userId);
