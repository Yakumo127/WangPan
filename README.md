...
> 说明：本节列出主要业务接口的典型用法，方便前后端联调与第三方系统集成。接口整体遵循统一风格：  
> - 认证后接口需要在请求头中携带 `Authorization: Bearer {token}`；  
> - 一般业务错误会以 `4xx` 状态码返回形如 `{ "message": "错误原因" }` 的 JSON；  
> - 管理员接口要求用户具备 `ROLE_ADMIN` 权限；  
> - 上传/下载等场景会返回二进制流或 `multipart/form-data`，其余场景以 JSON 为主。  
> 实际字段与边界条件可结合 `backend/src/main/java/com/filemanager/controller` 下的实现以及 `docs` 中的设计文档一起参考。
...
- **常见失败**：
  - 用户名或密码错误：返回 `400 Bad Request`，`{"message": "用户名或密码错误"}`；
  - 账号被禁用或锁定：返回 `400/403`，`{"message": "账号已禁用/已锁定"}`；
  - 验证码错误或过期（若启用验证码）：返回 `400`，`{"message": "验证码错误"}`。
...
> token 无效或已过期时，返回 `401 Unauthorized`。
...
- **典型错误**：
  - 旧密码不匹配：`400 Bad Request`，`{"message": "旧密码错误"}`；
  - 新密码不满足安全策略（如长度不足）：`400 Bad Request`，`{"message": "密码长度不足"}` 等。
...
> 中小文件可以直接用该接口一次性上传。超过配置阈值的大文件，前端通常会采用“先计算整文件哈希 → 调用秒传接口 → 分片上传 → 合并”的方式，详见下文“秒传与分片上传相关接口”。
...
> 删除为软删除：文件记录保留、标记为删除并进入回收站，用户配额会立即释放；物理文件与 Blob 的清理由后续任务或管理员操作触发。
...
> 前端通常通过 `window.location.href` 或隐藏 `<a>` 标签点击的方式使用该直链，配合浏览器原生下载体验，并避免 Axios 在大文件下载场景中的超时限制；后台会检查 Token 是否匹配当前用户且在有效期内。

#### 7. 文件移动与复制

- **移动文件**
  - **URL**：`POST /api/files/{fileId}/move`
  - **功能**：将指定文件移动到目标文件夹，可选地修改名称。
  - **请求体**：

    ```json
    {
      "folderId": 123,      // 目标文件夹 ID，可为 null 表示移动到根目录
      "newName": "可选的新文件名"
    }
    ```
...
- **复制文件**
  - **URL**：`POST /api/files/{fileId}/copy`
  - **功能**：复制文件到目标文件夹，可选地修改名称；底层尽量复用 Blob，避免重复占用磁盘。
  - **请求体**：
...
- **获取版本列表**
  - **URL**：`GET /api/files/{fileId}/versions`
  - **功能**：返回指定文件的所有历史版本（包含大小、创建时间等信息）。
- **按版本下载**
  - **URL**：`GET /api/files/download/{fileId}/version/{versionNo}`
  - **功能**：下载指定文件的某个历史版本。
- **版本回滚**
  - **URL**：`POST /api/files/{fileId}/versions/revert`
  - **功能**：基于指定版本创建一个新的版本并设置为当前激活版本，实现“回滚”效果。
  - **请求体** 示例：
...
> 回滚不会删除历史记录，而是追加一个指向旧 Blob 的新版本；配额会根据旧版本大小和当前版本大小进行调整。

#### 10. 秒传与分片上传相关接口（示意）

完整大文件上传典型流程：**计算整文件哈希 → 秒传检查 →（如不能秒传）分片上传 → 分片状态查询/断点续传 → 合并分片**。

- **秒传检查（当前用户空间）**
  - **URL**：`POST /api/files/exists`
  - **请求示例**：
...
- **秒传检查（全局 Blob）**
  - **URL**：`POST /api/files/exists-global`
  - **功能**：检查全局 Blob 是否已存在，用于跨用户复用存储。
...
- **查询分片状态（断点续传）**
  - **URL**：`GET /api/files/chunk/status`
  - **请求参数**：
    - `fileHash`：整文件哈希；
    - `totalChunks`：可选，用于返回缺失分片列表。
  - **响应示例**：
...
- **合并分片**
  ...
  - **响应**：与普通上传成功时的返回结构类似，包含 `fileId`、`filename`、`size`、`uploadTime` 等字段。

> 分片大小、最大文件大小由配置 `file.storage.chunk-size`、`file.storage.max-file-size` 以及后台系统设置决定。
...
- **请求参数**：
  - `parentId`：可选，父文件夹 ID。
- **返回数据**：简化 Folder 对象数组（包含 id、name、parentId、createTime 等）。

#### 3. 文件夹删除/重命名/移动/复制（示意）

- **删除文件夹**
  - **URL**：`DELETE /api/folders/{folderId}`
  - **功能**：删除指定文件夹（软删），目录及其子内容进入回收站。

- **重命名文件夹**
  - **URL**：`PUT /api/folders/{folderId}/rename`
  - **请求体**：
...
- **移动文件夹**
  - **URL**：`POST /api/folders/{folderId}/move`
  - **请求体**：
...
- **复制文件夹**
  - **URL**：`POST /api/folders/{folderId}/copy`
  - **功能**：复制整个目录树到指定父目录，可选新名称。

- **获取文件夹路径（面包屑）**
  - **URL**：`GET /api/folders/{folderId}/path`
  - **功能**：返回从根目录到当前目录的路径链，用于前端展示面包屑导航。
...
- **分页获取全量文件**
  - **URL**：`GET /api/files/admin/list`
  - **功能**：按创建时间倒序分页获取系统内所有文件，可按关键字与状态过滤。
  - **常用查询参数**：
    - `page`：页号，从 0 开始，默认 0；
    - `size`：每页大小，默认 20；
    - `keyword`：可选，按文件名等字段模糊匹配；
    - `status`：可选，`active` / `deleted` / `all`。

- **管理员下载任意文件**
  - **URL**：`GET /api/files/admin/download/{fileId}`
  - **功能**：下载系统中的任意文件（不受归属限制），用于排查或取证。

> 管理员还可以通过其他管理端接口调整文件配额、排期删除、触发 Blob GC 等，详细可以参考 `Admin*Controller` 与 `FileController` 中的实现。
...
- **功能**：获取当前用户回收站中的文件列表。
- **返回数据**：回收站文件列表（与 `files` 类似，增加删除时间等信息），可用于个人回收站页面展示。
...
> 管理员回收站还支持按时间窗口查询、按执行状态过滤等能力，便于排查与批量治理历史数据。
...
- ✅ 用户注册（用户名/邮箱唯一校验）：限制用户名长度与字符集，邮箱需唯一，避免重复注册。
- ✅ 用户登录（JWT 认证）：基于用户名 + 密码 +（可选）验证码登录，签发带过期时间的 JWT，用于访问后续接口。
- ✅ 管理员登录专用接口：独立的 `/api/auth/admin-login`，只允许具备 ADMIN 角色的用户登录，并在审计日志中以管理员身份记录。
- ✅ 图形验证码（旧版图片接口与新版 Base64 接口）：支持前端以 `<img>` 或 Base64 方式加载验证码图片，用于登录防刷。
- ✅ 登录失败次数累计与锁定机制：连续失败超过阈值后自动锁定账号，防止暴力破解。
- ✅ 密码修改：支持已登录用户修改密码，校验旧密码正确性与新密码复杂度。
- ✅ 个人资料管理（昵称、邮箱、头像）：允许用户更新基础资料，便于在文件列表、日志中展示更友好的名称。
- ✅ 角色管理（USER/ADMIN）：通过后台管理界面分配角色，前后端统一通过 `ROLE_*` 权限控制接口访问。

#### 2. 文件管理

- ✅ 文件上传（支持大文件）：提供单请求上传与分片上传两种模式，根据文件大小和配置自动选择合适策略。
- ✅ 文件下载（支持断点续传与多版本下载）：后端支持 Range 请求与多版本按版本号下载，前端可根据需要选择特定历史版本。
- ✅ 文件删除（软删除，进入回收站）：删除后文件并不会立即物理清除，而是进入回收站，可恢复或彻底删除。
- ✅ 文件重命名：允许在不改变物理文件路径的前提下修改展示名称，并处理同一目录重名情况。
- ✅ 文件移动（支持批量）：支持将文件在目录树中重定位，前端通过选择目标目录和多选文件实现一键批量移动。
- ✅ 文件复制（支持批量，支持跨目录复制）：复制时尽量复用底层 Blob，避免真正复制文件数据，仅创建新的元数据与版本记录。
- ✅ 文件搜索（基于名称等字段）：允许在当前用户空间内按关键字快速查找文件，结合前端筛选可实现更复杂的查询。
- ✅ 文件预览（图片、PDF 等）：通过 `/api/files/{id}/preview` 内联预览文件，前端根据 MIME 类型选择图片预览对话框或 PDF 预览页。
- ✅ 文件分享（下载直链 + 一次性 Token）：通过 `/download-url` 生成短期有效直链，避免暴露真实存储路径，满足简单分享需求。
- ✅ 批量操作（批量移动/复制/删除/下载）：列表支持勾选多条记录后一次性执行移动/复制/删除/下载等操作，减少重复点击。
- ✅ 文件版本管理（版本列表、指定版本下载与回滚）：每次覆盖上传都会形成新版本，用户可以查看版本历史、下载旧版本或通过追加新版本完成回滚。
- ✅ 秒传（按文件哈希检查已存在文件/Blob）：通过整文件哈希检测是否已经上传过该文件，已存在时秒级创建文件记录，极大缩短上传时间。

#### 3. 文件夹管理

- ✅ 创建文件夹（支持层级结构）：可在任意已有目录下创建子目录，形成多层级目录树。
- ✅ 删除文件夹（软删，进入回收站）：删除目录同时会软删其下所有子目录与文件，保持结构一致。
- ✅ 重命名文件夹：允许调整目录名称，而不影响其下内容结构。
- ✅ 移动文件夹：支持将整个目录树移动到新的父目录，避免逐个移动子项。
- ✅ 文件夹层级管理与树状导航：后端返回目录树结构，前端用树形控件展示，可折叠/展开各层级。
- ✅ 面包屑导航：根据当前目录路径展示面包屑，支持快速跳转到上级目录。
- ✅ 文件夹复制（按需扩展）：可复制整个目录结构到新位置，用于模板目录或项目复制场景。

#### 4. 回收站功能

- ✅ 个人回收站（每个用户独立）：用户只能看到自己删除的文件/文件夹，数据隔离清晰。
- ✅ 文件/文件夹恢复：一键恢复到原路径位置，若原路径已不存在则可按策略恢复到根目录或指定位置。
- ✅ 永久删除（从回收站彻底删除）：对用户而言彻底删除后将不再出现在个人视图中，仅在管理员全局视图中可见或被清理。
- ✅ 管理员回收站管理（可跨用户恢复/删除）：支持按用户、时间、大小等维度筛选回收站条目，集中治理空间。
- ✅ 批量恢复：适合用户误删多个文件后一次性恢复的场景。
- ✅ 批量删除：支持批量彻底删除占用空间大的历史文件，释放存储资源。
- ✅ 管理员排期删除与冷静期配置：可为敏感文件设置冷静期，在到期前仍可恢复，到期后由任务自动清理。
...
- ✅ 用户搜索
- ✅ 用户创建/编辑/删除
- ✅ 用户状态管理（启用/禁用/锁定）：结合登录失败策略，对异常账号进行管控。
- ✅ 用户角色与权限管理：支持动态调整用户角色，用于分配管理员或收回管理员权限。
- ✅ 配额管理（调整单个用户存储配额）：可为不同用户分配不同的存储空间上限。

#### 2. 系统管理

- ✅ 系统配置管理（上传策略、回收站策略、备份策略等）：支持在线修改关键参数，部分配置可即时生效。
- ✅ 存储配额全局配置（默认配额、最大文件大小、分片大小等）：通过系统配置表统一管理上传与存储策略。
- ✅ 用户操作日志查看与导出：可按时间、用户、操作类型筛选审计日志，支持导出用于合规留痕。
- ✅ 系统监控面板（健康检查、存储使用等）：结合 Actuator 与自定义指标，展示整体健康与资源使用情况。
- ✅ 数据备份与清理策略配置：配置周期性备份与过期数据清理策略，减少人工干预。

#### 3. 回收站管理

- ✅ 全局回收站浏览：管理员可以统一查看所有用户的回收站条目。
- ✅ 恢复任意用户的文件/文件夹：用于处理用户误删或合规调查后的恢复需求。
- ✅ 永久删除指定文件/文件夹：管理员可以对特定敏感或过期数据执行最终删除。
- ✅ 一键清空所有回收站（结合配额与 Blob 清理）：适用于定期“年终大扫除”等场景。
...
- ✅ 密码加密存储
- ✅ 用户角色与权限分级管理
- ✅ 防止 SQL 注入（基于 JPA 与参数化查询）：不直接拼接 SQL 字符串。
- ✅ 文件访问控制（按用户/管理员区分）：所有文件读写操作都会验证当前用户是否有权访问。
- ✅ 审计日志记录（关键操作全记录）：对下载、预览、删除、设置修改等操作统一记录，方便追踪。
- ✅ 下载直链 Token 短期有效、防重放设计：直链 Token 使用单独密钥与短 TTL，通过后端校验防止被恶意复用。

#### 2. 性能优化

- ✅ Redis 缓存系统配置等热点数据
- ✅ 文件分片上传与断点续传：通过分片与状态查询接口，在网络不稳定的情况下也能可靠上传大文件。
- ✅ 秒传（按文件哈希快速创建文件）：避免重复上传相同内容，节约带宽与时间。
- ✅ 图片缩略图生成与缓存：在列表中优先展示小尺寸缩略图，减少图片加载时间。
- ✅ 数据库索引优化（常用查询字段建索引）：提升列表、搜索与统计类查询性能。
- ✅ 前端懒加载、列表分页与虚拟滚动（按需）：提升大数据量目录下的滚动与渲染体验。

#### 3. 用户体验

- ✅ 响应式布局（适配常见桌面分辨率）
- ✅ 拖拽上传
- ✅ 上传进度与上传队列管理：实时展示每个上传任务的进度，并支持暂停/恢复/重试。
- ✅ 实时搜索与过滤：在当前列表或回收站中按关键字即时过滤，提高定位效率。
- ✅ 多语言支持（可扩展）
- ✅ 深色模式（可扩展/实验性）
