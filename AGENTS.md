...
1. 用户授权优先：任何代码或文件的修改操作必须事先得到用户明确同意。
2. 修改必须生成报告：每次代码或文件变更前，都需在 docs 目录下对应位置创建一份完整报告。
3. 报告文件命名要求：文件名必须包含“分钟级”时间戳，用于精确追踪。
4. 报告内容要求：必须全面，报告需包含且不限于：（问题成因、排查过程与历史、修复建议、实际采取的修复操作）必须完整、详尽，不得删减。
5. 文档操作记录日志：docs 目录下需维护一个总记录文件，记录每一次文档变更：何时执行、因何执行、做了什么操作、文件最终存放位置
6. 变更后必须同步仓库：文档生成后需同步到远端仓库，所有更改必须提交（commit + push）。
7. GitHub 推送要求：推送需要使用临时代理，推送描述必须遵守既定语言规则,推送所有文件。
8. 临时代理
```
